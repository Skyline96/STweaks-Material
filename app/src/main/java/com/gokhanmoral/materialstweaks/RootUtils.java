...
class RootUtils {
    ...
    static boolean canRunRootCommandsInThread() {
        boolean isOk = false;
        String line;
    ...
    static String executeRootCommandInThread(String command) {
        ...
        try {
            if (null != command && command.length() > 0) {
                String scriptSeparator = "[%nexttweaks%]";
                ...
                while (!finished) {
                    if (consoleOut.ready()) {
                        ...
                        // Log.d(LOG_TAG, "--> not ready for stdout or stderr!");
                          try { Thread.sleep(1); }
            ...
            Log.w(LOG_TAG + "execRootCommandInThread", "Running command failed: " + command, ex);
        }
